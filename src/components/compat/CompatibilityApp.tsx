...
export default function CompatibilityApp({ embeddedMode = false }) {
  ...
  const { count, increment } = useAnalysisCounter()
  ...
  const handleKakaoShare = useCallback(() => {
    ...
    const label = r?.overall?.label   ?? ''
    const url   = window.location.href
    const origin = typeof window !== 'undefined' ? window.location.origin : 'https://sajulab.com'
    const ogImg = `${origin}/api/og?nameA=${encodeURIComponent(nameA)}&nameB=${encodeURIComponent(nameB)}&score=${score}&label=${encodeURIComponent(label)}`
    try {
      if (typeof Kakao !== 'undefined') {
        ...
        Kakao.Share.sendDefault({
          objectType: 'feed',
          content: {
            title: `${nameA} × ${nameB} 역학 궁합 분석`,
            description: `종합 점수: ${score}점 (${label}) | 사주·자미두수·점성술 통합 분석`,
            imageUrl: ogImg,
  ...
  const ShareButtons = ({ dark = false }) => (
    <>
      <button
        onClick={handleKakaoShare}
        className={`flex items-center gap-1 text-xs px-3 py-1.5 rounded transition-colors ${dark
          ? 'bg-yellow-400 hover:bg-yellow-300 text-gray-900 font-semibold'
          : 'bg-yellow-400 hover:bg-yellow-300 text-gray-900 font-semibold border border-yellow-500'}`}
      >
        <span>💬</span><span>{t('kakao_share')}</span>
      </button>
      <button
        onClick={handleShare}
        className={`text-xs px-3 py-1.5 rounded transition-colors ${dark
          ? 'text-ink-400 hover:text-white border border-ink-700 hover:border-ink-500'
  ...
  if (embeddedMode) {
    return (
      <div className="bg-gradient-to-br from-indigo-950 via-indigo-900 to-purple-900 text-white">
          ...
          <div className="text-center mb-6">
            ...
            {count !== null && (
              <p className="text-indigo-400 text-xs mt-2">
                🔮 <span className="font-semibold text-indigo-200">{count.toLocaleString()}</span>{t('counter_suffix')}
              </p>
            )}
          </div>
        ...
        {result && (<div className="bg-ink-50"><div className="max-w-4xl mx-auto px-4 py-6"><div className="flex justify-end gap-2 mb-4"><ShareButtons /></div><ResultDisplay data={result} /></div></div>)}
      </div>
    )
  }
    ...
    <div className="bg-ink-50 min-h-screen">
      <div className="bg-ink-900 text-white">
        <div className="max-w-4xl mx-auto px-4 py-5 flex items-center justify-between">
          ...
          {result && (<div className="flex items-center gap-2"><ShareButtons dark /></div>)}
        </div>
      </div>
      {result && (<div className="md:hidden fixed bottom-0 left-0 right-0 z-30 bg-white border-t border-gray-200 flex items-center justify-around px-4 py-2 shadow-lg print:hidden">
        <button onClick={handleKakaoShare} className="flex flex-col items-center gap-0.5 text-gray-800 hover:text-yellow-600 text-xs"><span className="text-lg">💬</span><span>{t('kakao_share')}</span></button>
        <button onClick={handleShare} className="flex flex-col items-center gap-0.5 text-gray-600 hover:text-indigo-600 text-xs"><span className="text-lg">{copied ? '✓' : '🔗'}</span><span>{copied ? t('copied') : t('share')}</span></button>
        <button onClick={() => window.print()} className="flex flex-col items-center gap-0.5 text-gray-600 hover:text-indigo-600 text-xs"><span className="text-lg">🖨</span><span>{t('pdf')}</span></button>
        <button onClick={handleReset} className="flex flex-col items-center gap-0.5 text-gray-600 hover:text-indigo-600 text-xs"><span className="text-lg">✦</span><span>{t('new_analysis')}</span></button>
      </div>)}
